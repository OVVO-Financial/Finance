VN.Utility <- function (n,q,required.return,x,upside.target=NULL,downside.target=NULL){
  ...
  output.2<- -downside.target.utility + (condition.2 - downside.target)^n - U0.n
  
  
  ## For x < upside target but greater than required return, positive utility of upside target less downside deviation (CONCAVE)
  condition.3 <- x[x<=upside.target & x>required.return]
  output.3 <- upside.target.utility - (upside.target - condition.3)^q + U0.p
  ...
  output.4 <- upside.target.utility + (condition.4 - upside.target)^q + U0.p
  
  
  output=c(output.1,output.2,output.3,output.4)
  ...
  text(downside.target,min(output)/2,"Downside Target",pos=3,col='red')
  text(upside.target,max(output)/2,"Upside Target",pos=3,col='green')
